...

{
    ...
    public HibernateConfig() {
        ...
        {
            // Throws :  The last packet sent successfully to the server was 0 milliseconds ago.
            //           The driver has not received any packets from the server.
            System.out.println("Your Port is not correct");
            e.printStackTrace();
        }
        ...
        {
            // Throws :  Could not obtain connection to query metadata : Access denied for user 'foot'@'localhost'
            System.out.println("Your username is not correct");
            e.printStackTrace();
        }
        try
        {
            config.setProperty("hibernate.connection.password", "arellana");
        }
        catch (Exception e)
        {
            // Throws :  Access denied for user 'root'@'localhost' (using password: YES)
